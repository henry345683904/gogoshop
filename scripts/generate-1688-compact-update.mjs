...
select '1688-'||x.id,convert_from(decode(x.title_hex,'hex'),'utf8'),'1688 Imports','',x.price,0,x.price,x.stock,0,false,'','[]'::jsonb,convert_from(decode(x.vendor_hex,'hex'),'utf8'),'1688-'||x.id,'1688, imported, draft','https://detail.1688.com/offer/'||x.id||'.html','CNY',x.price,'[]'::jsonb,'[]'::jsonb,now()
from(values${hidden})x(id,title_hex,price,stock,vendor_hex)
on conflict(id)do update set title=excluded.title,stock=excluded.stock,vendor=excluded.vendor,source_url=excluded.source_url,source_price=excluded.source_price,updated_at=now();
